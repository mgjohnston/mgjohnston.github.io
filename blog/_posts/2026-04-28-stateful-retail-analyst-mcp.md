...
- The server is **stateful**: it enforces a curriculum (`about` → `describe_table` → `query`) so the model can't query a table it hasn't been briefed on, and self-corrects when it tries to.
...
Our Claude.ai tenant can SSO into a read-only MCP server I deployed alongside a daily Snowflake export. It has been remarkably effective at drawing together disparate data sources into coherent analyses across our retail domain, and has let colleagues at every level interact with our data in real time without waiting for BI nor analyst capacity.

I employed the stateful nature of MCPs to gate data access behind enforced data discovery. This leads to SQL generation and responses which are grounded in business logic and real data.
...
The model correctly understands Uber means Jollyes' Zoomies last-mile channel, pulls sales data across channels and compares across time and the whole estate, as well as considering other meanings of 'performing' such as ATV and customer reviews. Moreover, it knows to provide a Jollyes-ready response with a % as well as absolute revenue number.
...
The data choices were deliberate: small enough to fit in memory on a cheap VM, PII-clean, and well-described.

## The trick: about → describe_table → query

Naïve "natural language to SQL" works in demos, but cannot work in an enterprise context. The models are able to generate valid SQL, but production data is full of traps that aren't in the schema, meaning that the generated SQL misses business context. For example:

- Our fiscal calendar sometimes has 53-week years. Therefore, YoY comparisons need `dim_date.last_year_date`, not `today - 365`.
- We always use like-for-like in our analyses, but what does that mean at Jollyes?
- `fact_stock` double-counts inventory across parent/child SKU pairs unless you dedupe through `dim_sku_links`.
...
So, the **MCP server enforces a curriculum** teaching the agent about Jollyes business and data quirks:

1. **`about()`** - must be called first. Returns current fiscal state, the table catalogue, the canonical join graph, and the business context (e.g. EDLP, loyalty, livestock). ~6k tokens, paid once per session.
2. **`describe_table(name)`** - must be called for every table the model wants to query. Returns the schema, sample rows, and - crucially - per-table column notes, useful patterns, and warnings we've accumulated from working with Jollyes data. Only runs if `about()` has been called.
...
Large prompts don't simply cost more tokens - they dilute signal. Irrelevant context is distraction in an attention-focussed model.
...
- **Updates are centralised.** When I add a new table or come across a new common mistake, I update the MCP server once and don't need to distribute a new skill file.
